...
public class CheckMove {
	...
	for (int k = 0; k < ChessMain.currentUser.length; k++) {
	    if (ChessMain.chessBoard[GetMove.move[1]][GetMove.move[0]] == ChessMain.currentUser[k]) {
		return figures(k);
	    }
	}
	...
	// нельзя бить "своих"
	if (ChessMain.chessBoard[GetMove.move[3]][GetMove.move[2]] != null) {
	    for (int i = 0; i < ChessMain.currentUser.length; i++) {
		if (ChessMain.chessBoard[GetMove.move[3]][GetMove.move[2]] == ChessMain.currentUser[i]) {
	...
	//не открывается ли корль для шаха
	if (isOpenToShah()) {
	    return false;
	}
	...
	String[] color2;
	if (ChessMain.currentUser == ChessMain.PIECESWHITE) {
	    color2 = ChessMain.PIECESBLACK;

	} else {
	    color2 = ChessMain.PIECESWHITE;
	}
	// найти своего короля
	int digit, letter = 0;
	loop: for (digit = 0; digit < 8; digit++) {
	    for (letter = 0; letter < 8; letter++) {
		if (ChessMain.chessBoard[digit][letter] == ChessMain.currentUser[5]) {
		    break loop;
		}
	    }
	}
	...
	if (Math.abs(letter - GetMove.move[0]) == Math.abs(digit - GetMove.move[1])) {
		...
		if (digit < GetMove.move[1]) {
			...
			if (ChessMain.chessBoard[j][i] != null) {
			    if (ChessMain.chessBoard[j][i] == color2[3] || ChessMain.chessBoard[j][i] == color2[4]
				    || ChessMain.chessBoard[j][i] == color2[6]) {
				shah = true;
				break;
			    } else {
				break;
	...
	// проверка не открывается ли король для ладьи
		...
		if (ChessMain.chessBoard[i][GetMove.move[0]] != null) {
		    if (ChessMain.chessBoard[i][GetMove.move[0]] == color2[1]
			    || ChessMain.chessBoard[i][GetMove.move[0]] == color2[4]
			    || ChessMain.chessBoard[i][GetMove.move[0]] == color2[8]) {
			shah = true;
			break;
		    } else {
			break;
		...
		if (ChessMain.chessBoard[GetMove.move[1]][j] != null) {
		    if (ChessMain.chessBoard[GetMove.move[1]][j] == color2[1]
			    || ChessMain.chessBoard[GetMove.move[1]][j] == color2[4]
			    || ChessMain.chessBoard[GetMove.move[1]][j] == color2[8]) {
			shah = true;
			break;
		    } else {
			break;
